#!/usr/bin/env python3
"""업로드한 LSTM·UPC bundle을 Colab에서 실행하고 결과 ZIP을 만든다."""
...
BUNDLE_PATH = Path("/content/gecos_lstm_upc_smoke_bundle.zip")
WORKSPACE = Path("/content/gecos")
OUTPUT_ZIP = Path("/content/gecos_lstm_upc_smoke_outputs.zip")
...
def main() -> int:
    ...
    from scripts.lstm_contract import load_lstm_smoke_config
    from scripts.run_lstm_upc_smoke import run_lstm_upc_smoke

    config = load_lstm_smoke_config(
        WORKSPACE / "configs" / "lstm_upc_smoke_milan_nov2013.json",
...
if __name__ == "__main__":
    exit_status = main()
    if exit_status:
        raise RuntimeError("LSTM·UPC smoke의 필수 gate를 통과하지 못했습니다.")
