...
- 문서 상태: 학습용 부분 재현 종료 (LSTM 전체 Train·Validation 완료)

이 문서는 처음 등록한 전체 재현 계획과 실행 계약을 보존한다. 최종적으로는 모든
항목을 실행하지 않고 LSTM 전체 Train·Validation까지를 학습용 종료 지점으로
선택했다. 현재 상태와 중단 근거는
[학습용 논문 재현 최종 정리](14-study-reproduction-conclusion.md)를 우선한다.
...
이 절은 사전 계획을 결과에 맞춰 축소하지 않기 위해 원문을 보존한다. 최종 프로젝트는
RCTL 전체 학습, RCC 비교와 10,000셀 확장 전에 종료했으므로 이 계획의 모든 항목을
완료한 것은 아니다.
...
| 수준 | 의미 | 처음 계획 | 최종 판정 |
|---|---|---|---|
| 구조 재현 | 논문의 데이터 흐름과 알고리즘 구조를 구현 | 필수 | 데이터·UPC 완료, RCTL은 구조 감사까지 완료 |
| 설정 재현 | 공개된 hyperparameter를 적용 | 필수 | 공개값과 구현 가정을 분리해 적용 |
| 수치 재현 | 논문 표의 수치와 오차 범위 내 일치 | 참고 목표 | 미완료, 수치 일치를 주장하지 않음 |
| 확장 재현 | 10,000셀 및 추가 모델 비교 | 선택 | 미실행 |
...
11. 같은 계약의 중앙 900셀 RCTL UPC on/off 비교 — **의도적으로 미실행**
12. RCC 최소 ablation — **의도적으로 미실행**
13. 최적 구성의 10,000셀 단일 확장 실험 — **의도적으로 미실행**

10단계 뒤에는 데이터·clustering·평가·구조 감사와 하나의 전체 모델 비교라는 학습
목표가 충족됐다. 반면 11~13단계는 미확정 RCTL 구조를 임의로 선택해야 하고 계산
비용에 비해 새로운 학습이 제한적이므로 종료 범위에서 제외했다. 잠긴 LSTM Test
평가도 과거 raw smoke의 Test 노출 이력을 고려해 실행하지 않았다. 이는 pipeline
실패가 아니라 명시적인 중단 결정이며 자세한 평가는
...
## 13. 처음 정의한 완료 조건과 최종 종료 판정

처음에는 다음 조건을 모두 만족하면 1차 핵심 재현이 완료된 것으로 정의했다.
...
최종적으로 원본 데이터 재생성, 자동 계약 검사, UPC 비교, RCTL 차이 설명, LSTM
seed별 Validation 결과와 provenance 보존은 완료했다. 그러나 중앙 900셀 RCTL·RCC
성능과 논문의 전체 GECOS 결과는 생성하지 않았으므로 위 의미의 **1차 핵심 재현은
미완료**다.
