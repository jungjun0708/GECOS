...
현재 UPC 초기 그룹 테스트를 포함한 전체 합성 테스트 36개가 모두 통과한다.
...
후속 단계에서 이 900셀과 전체 10,000셀의 평일 peak hour를 계산해 UPC 초기 24개
그룹을 만들었다. 명시된 Algorithm 1과 논문 Fig. 4가 정확히 일치하지 않아 시간대,
1시간 집계, scaling과 숨은 기간 선택 가설까지 조사했으며 아직 RCTL 학습으로
넘어가지 않는다. 결과는
