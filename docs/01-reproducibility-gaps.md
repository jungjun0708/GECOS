...
- 영향: 논문의 서술형 알고리즘, 기간 선택과 Fig. 4 생성 코드 중 적어도 하나에
  ...
  누수가 생길 수 있다.
- 방침: Algorithm 1 구현, Train 기간만 사용하는 구현과 Fig. 4 진단 가설을 서로
  다른 이름으로 보존한다. 진단 가설을 주 결과에 사용하지 않으며 정확한 일치라고
  표현하지 않는다. 원저자 UPC·전처리 코드 또는 그림 생성 자료가 확인되기 전까지
  gap을 해결된 것으로 처리하지 않는다.
