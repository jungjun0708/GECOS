...
## 1.1 학습용 종료 시점의 최종 분류

프로젝트 종료는 gap을 모두 해결했다는 뜻이 아니다. 각 항목의 원래 근거와 상태는
아래에 보존하고, 현재 판단을 다음 두 묶음으로 요약한다.

| 최종 분류 | 의미 | gap ID |
|---|---|---|
| 저장소 내부 방침 고정 | 공개 정보가 부족해도 이 프로젝트의 처리·평가 계약은 결정됨 | `UPC-01~05`, `RCTL-06`, `DATA-01`, `DATA-03~04`, `DATA-06`, `METRIC-01~03`, `REPO-03` |
| 내부 진단 완료·외부 확인 불가 | 구현 후보나 감사 결과는 있지만 원저자의 정확한 설정으로 확정할 수 없음 | `UPC-06~07`, `RCTL-01~05`, `LSTM-01~03`, `DATA-02`, `DATA-05`, `METRIC-04~06`, `REPO-01~02` |

두 번째 묶음은 추가 parameter 탐색이나 결과 맞추기로 닫지 않는다. RCTL 전체
학습·RCC ablation·10,000셀 확장과 최종 Test는 이러한 한계와 학습 대비 비용을
고려해 의도적으로 실행하지 않았다. 종료 결정은
...
- 상태: `학습 범위 결정 완료·저자 순서는 외부 정보 필요`
...
- 구현 결과: `train_only`은 전체 10,000셀과 중앙 900셀 모두 100% 같았다.
  ...
  사후 선택하지 않는다.
- 방침: 추가 순서를 탐색하지 않고 오름차순 재현 경로를 유지한다. 누수 없고 순서에
  100% 안정적인 `train_only`만 주 모델 학습에 허용한다. 전체 월은 clustering
  민감도로 보존하되 신경망 학습을 보류하고, Fig. 4 probe는 계속 금지한다. 순서 불변
  대안은 필요할 때 별도 비재현 확장 실험으로만 다룬다. clustering manifest의 전역
  `false` gate는 보존하고 별도 정책에 프로토콜별 허용 여부를 기록한다.
- 상세 기록: [UPC PCC 기반 최종 2개 클러스터](09-upc-pcc-final-clusters.md) 및
  [UPC 순서 민감도 검토와 프로토콜별 학습 정책](10-upc-order-training-policy.md)
...
- 상태: `RCTL smoke·LSTM 전체 학습 환경 결정 완료·저자 환경은 외부 정보 필요`
...
- 구현 결과: Tesla T4 15,360MiB, Python 3.13.15, NumPy 2.1.3, TensorFlow 2.20.0,
  ...
  모두 같았다. LSTM 전체 학습은 같은 환경에서 9개 job을 실행했고, 집계를 두 번
  수행했을 때 실행시각을 제외한 다섯 핵심 산출물의 SHA-256이 모두 같았다.
